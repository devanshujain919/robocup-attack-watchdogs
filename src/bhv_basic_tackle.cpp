...
Bhv_BasicTackle::execute( PlayerAgent * agent )
{
    ...
    double tackle_prob = wm.self().tackleProbability();
      ...
      /*If the ball is outside the penalty box 
        and have received no card till now*/
    ...
    const int self_min = wm.interceptTable()->selfReachCycle(); /*\brief get minimal ball gettable cycle for self with stamina exhaust
                                                                  \return cycle value to get the ball*/

    const int mate_min = wm.interceptTable()->teammateReachCycle(); /*\brief get minimal ball gettable cycle for teammate
                                                                      \return cycle value to get the ball*/

    const int opp_min = wm.interceptTable()->opponentReachCycle();  /*\brief get minimal ball gettable cycle for opponent
                                                                      \return cycle value to get the ball*/

    const Vector2D self_reach_point = wm.ball().inertiaPoint( self_min ); /*\brief estimate reach point
    ...
    {
        const Ray2D ball_ray( wm.ball().pos(), wm.ball().vel().th() ); /* Ray2D()
                                                                        : M_origin( 0.0, 0.0 )
                                                                        , M_direction( 0.0 )
                                                                      { }*/
